$(function () {
    ...
    $('#Form1').attr("action", "/SalesOrder/" + action);
...
function getUnitOnProduct(index) {
    ...
    $.ajax({
        ...
        success: function (validUnit) {
            ...
            var ddl = document.getElementById("Details_" + index + "_ConversionIDTemp");
...
function addTooltip() {
    ...
    if (mode == "Detail" || mode == "Create") {
        var bsID = $("#hdnBookingSalesID").val();
        var bsCode = $("#lblBookingSalesCode").text();

        var bsLink = "/BookingSales/Detail?key=" + bsID;

        $("#lblBookingSalesCode").replaceWith('<a href="' + bsLink + '">' + bsCode + '</a>');
    ...
    var noOfRows = $("#customerReturnGrid tbody tr").length;

    for (x = 0; x < noOfRows; x++) {
        var customerReturnID = $("#customerReturnDetails_" + x + "_ID").val();
        var customerReturnCode = $("#customerReturnDetails_" + x + "_Code").text();
        var customerReturnLink = "/CustomerReturn/Detail?key=" + customerReturnID;

        $("#customerReturnDetails_" + x + "_Code").replaceWith('<a href="' + customerReturnLink + '" style="color: #505abc;">' + customerReturnCode + '</a>');
    }
...
function onSelectCustomer(data) {
    var invoiceBalance = data.InvoiceBalance;
    console.log("Invoice balance = " + invoiceBalance);
    var creditLimit = data.CreditLimit;
    console.log("Credit Limit = " + creditLimit);
    var onCreditHold = data.OnCreditHold;
    console.log("On Credit hold = " + onCreditHold);
    var blackList = data.BlackList;
    //if (invoiceBalance > creditLimit && onCreditHold) {
    //    alert("Customer is over credit limit! Please contact a supervisor");
    //    $("#txtCustomerName").val('');
    //    return;
    //}
    if (blackList) {
        alert("Customer is BLACK LIST! Please contact a Administrator");
        $("#txtCustomerName").val('');
        return;
    }
    $("#hdnCustomerID").val(data.ID);
    $("#txtCustomerName").val(data.Name);
    $("#hdnSalesmanID").val(data.SalesRep);
    $("#txtSalesReference").val(data.SalesRepName);
...
function recalcShipping() {
    ...
    var shipTax = parseFloat($("#txtShippingTax").val());
    var taxType = $("#ShippingTaxCode").val();

    if (taxType == 2) {
        shipTax = shipCost * 0.1;
    }
    else {
        shipTax = 0;
    }

    $("#txtShippingTax").val(shipTax);

    recalcShippingTotal();
}

function recalcShippingTotal() {
    var shipCost = parseFloat($("#txtShippingCost").val());
    var shipTax = parseFloat($("#txtShippingTax").val());
    var shipTotal = shipCost + shipTax;
    $("#txtShippingTotal").val(shipTotal);
    updateShippingTotal();
}

function updateShippingTotal() {
    var shipTotal = $("#txtShippingTotal").val();
    $("#txtGridShipping").text(shipTotal.toLocaleString('en-US'));
    recalcSOTotals();
}
...
function priceLevelChange(index) {
    ...
    $.ajax({
        ...
        data: "{productID: " + productID + ", priceLevelID: " + priceLevelID + ", taxType: " + taxTypeID + ", warehouseID: " + warehouseID + "}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
...
function unitChange(index) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#Details_" + index + "_Price").val(oldPrice);
            calc(index);
        }
    });
}
...
function calc(index) {
    ...
    $("#Details_" + index + "_Total").val(total.toLocaleString('en-US'));

    recalcSOTotals();

    //addUrlWorkOrder(index)
...
function recalcSOTotals() {
    ...
    var shipTotal = parseFloat($("#txtShippingTotal").val());
    total = total + shipTotal;
...
function addUrlWorkOrder(index) {
    var qtyAvail = parseFloat($("#Details_" + index + "_StockAvailable").text());
    console.log("qty available= " + qtyAvail);
    var qty = parseFloat($("#Details_" + index + "_Quantity").text().replaceAll(',', '').replaceAll('.', ','));
    console.log("qty= " + qty);
    if (qty > qtyAvail) {
        var prodID = $("#Details_" + index + "_ProductID").val();
        var Qty = parseFloat($("#Details_" + index + "_Quantity").text().replaceAll(',', '').replaceAll('.', ','));
        var salesorderID = $("#hdnID").val();
        var woCode = $("#Details_" + index + "_WorkOrderCode").text();
        console.log("WoCode= " + woCode);
        if (woCode == '') {
            $("#Details_" + index + "_WorkOrderCode").text('Order');
            var woOrder = $("#Details_" + index + "_WorkOrderCode").text();
            var woLink = "/WorkOrder/Create?productID=" + prodID + "&Qty=" + Qty + "&salesorderID=" + salesorderID;
            $("#Details_" + index + "_WorkOrderCode").replaceWith('<a href="' + woLink + '">' + woOrder + '</a>');
        }
        else {
            var workorderID = $("#Details_" + index + "_WorkOrderID").val();
            var workorderCode = $("#Details_" + index + "_WorkOrderCode").text();
            var workorderLink = "/WorkOrder/Detail?key=" + workorderID;

            $("#Details_" + index + "_WorkOrderCode").replaceWith('<a href="' + workorderLink + '" style="color: #505abc;">' + workorderCode + '</a>');
        }
