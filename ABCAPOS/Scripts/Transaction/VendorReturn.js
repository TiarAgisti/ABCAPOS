...
function addTooltip() {
    var noOfRows = $("#returnGrid tbody tr").length;

    for (x = 0; x < noOfRows; x++) {
        var doID = $("#returnDetails_" + x + "_ID").val();
        var doCode = $("#returnDetails_" + x + "_Code").text();
        var doLink = "/VendorReturnDelivery/Detail?key=" + doID;

        $("#returnDetails_" + x + "_Code").replaceWith('<a href="' + doLink + '" style="color: #505abc;">' + doCode + '</a>');
    }

    var noOfRows = $("#creditGrid tbody tr").length;

    for (x = 0; x < noOfRows; x++) {
        var billID = $("#creditDetails_" + x + "_ID").val();
        var billCode = $("#creditDetails_" + x + "_Code").text();
        var billLink = "/BillCredit/Detail?key=" + billID;
        console.log(billCode + billCode);
        $("#creditDetails_" + x + "_Code").replaceWith('<a href="' + billLink + '" style="color: #505abc;">' + billCode + '</a>');
    }
}
...
function onSelectVendorBillCredit(data) {
...
function onSelectProductBillCredit(data, index) {
    $.ajax({
        type: "POST",
        async: true,
        url: "/Webservice.asmx/GetUnitsByProductID",
...
function calc(index) {
    ...
    for (x = 0; x < rowCount; x++) {
        ...
        total += st;
        var tax = parseFloat($("#Details_" + x + "_TotalPPN").text().replaceAll(',', '').replaceAll('.', ','));
        taxTotal += tax;
    }
    
    var strTotal = total.toLocaleString().replaceAll('.', ',');
    var strTotalTax = taxTotal.toLocaleString().replaceAll('.', ',');
    $('#txtGrandTotal').val(total);
    $('#txtTaxValue').val(strTotalTax);
}
...
function isQuantityChange(index) {
    ...
    if (qty < qtyInput) {
        alert("Anda menginput angka lebih besar daripada Qty Retur");
